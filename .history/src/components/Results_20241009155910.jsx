...
const ResultsSlideshow = ({ images = [
  ...
  return (
    <div className="bg-customBlue p-6 w-full mx-auto">
      ...
      <div className="h-[200px] sm:h-[300px] md:h-[350px] max-w-[70%] mx-auto relative"> {/* Reduced width */}
        <Slider {...settings}>
          {images.map((image, index) => (
            <div key={index} className="px-1 sm:px-2 focus:outline-none"> {/* Reduced padding */}
              <div className="transform transition-all duration-300 hover:scale-110 border-2 border-gray-300 shadow-lg rounded-md overflow-hidden">  {/* Zoomed in on hover, reduced frame thickness */}
                <img
                  src={image}
                  alt={`Result ${index + 1}`}
                  className="w-full h-[150px] sm:h-[250px] md:h-[300px] object-cover scale-105"  // Added scale to zoom in
                />
              </div>
            </div>
          ))}

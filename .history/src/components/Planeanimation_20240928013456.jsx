...
import './PlaneAnimation.css'; // Custom styles for additional animations

// Plane Component with Three.js
const Plane = () => {
  const obj = useLoader(OBJLoader, '/Plane.obj'); // Replace with your .obj file path
  ...
  useEffect(() => {
    if (planeRef.current) {
      gsap.fromTo(planeRef.current.position, 
        { x: -5, y: 0 }, // Start from left
        { 
          x: 0, // Move to center
          scale: 1.5, // Zoom in
          duration: 3, 
          ease: 'power1.inOut',
          onComplete: () => {
            // Drop info after reaching center
            gsap.to('.info', {
              y: '0%',
              opacity: 1,
              duration: 1,
              ease: 'bounce.out',
              onComplete: () => {
                // Move plane to the right after dropping info
  ...
  return <primitive ref={planeRef} object={obj} scale={0.5} />;
};
...
const PlaneAnimation = () => {
  ...
  return (
    <div className="relative h-screen overflow-hidden bg-gradient-to-r from-customBlue to-blue-600">
      ...
      {/* Dropping Text */}
      <div className="absolute bottom-20 left-1/2 transform -translate-x-1/2 text-center z-20 info opacity-0">

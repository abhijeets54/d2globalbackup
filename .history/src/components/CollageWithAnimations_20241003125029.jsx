...
const CollageWithAnimations = () => {
  ...
  useEffect(() => {
    ...
    const startTyping = () => {
      setDisplayedText(''); // Clear the previous text

      // Ensure we have a valid text before starting the typing animation
      if (texts[textIndex]) {
        typingInterval = setInterval(() => {
          if (charIndex < texts[textIndex].length) {
            ...
            }, 1500); // Adjust pause time if needed
          }
        }, typingSpeed);
      }
    };
  ...
  }, [textIndex, texts]); // Re-run when the textIndex or texts array changes

  ...
  return (
    <div className="relative w-full max-w-4xl mx-auto">
      ...
      >
        ...
        {/* Animated dropping pins with larger text and boxes */}
        {Object.entries(countries).map(([country, position], index) => (
          ...
          >
            <motion.circle
              cx={position.x}
              cy={position.y}
              r="10" // Larger pin size
              fill="#3B82F6"
              whileHover={{ scale: 1.3 }}
              whileTap={{ scale: 0.9 }}
            />
            <motion.path
              d={`M${position.x},${position.y - 10} L${position.x},${position.y + 10}`} // Adjusted for larger circle
              stroke="#3B82F6"
              strokeWidth="3"
              initial={{ scale: 0 }}
              animate={{ scale: 1 }}
            ...
            {/* Box with country name */}
            <motion.rect
              x={position.x - 40} // Adjusted x position to center the rect around the text
              y={position.y - 35} // Adjusted y position to place the rect above the circle
            ...
            <text
              x={position.x}
              y={position.y - 20} // Positioning text within the rect
              textAnchor="middle"
              className="text-sm md:text-base lg:text-lg fill-white font-semibold" // Increased text size and made it responsive
            >
              {country}
            </text>
          </motion.g>

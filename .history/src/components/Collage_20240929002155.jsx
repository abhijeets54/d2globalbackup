...
function Collage() {
  ...
  useEffect(() => {
    ...
    const nextSlide = () => {
      ...
      if (activeElement && nextButton) {
        nextButton.click(); // Move to the next slide
      }
    };

    const firstSlideAnimationComplete = () => {
      nextSlide(); // Trigger the next slide after the animation ends
    };

    gsap.delayedCall(6, firstSlideAnimationComplete); // Fallback in case the animation timing doesn't work
  }, []);
  ...
  return (
    <div className="relative">
      <div
        ...
        data-bs-interval="6000" // Set the interval to ensure transition time allows for the plane animation
      >
        {/* Carousel Indicators */}

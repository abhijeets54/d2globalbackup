          ...
          <div className="flex flex-col lg:flex-row justify-between items-center relative min-h-[80px]">
            ...
            <div className="flex items-center relative flex-shrink-0 w-full lg:w-auto mb-4 lg:mb-0">
              ...
              <button
                ...
                className="hexagon-logo absolute w-[80px] h-[92px] bg-[#051b38] top-4 left-4 md:w-[100px] md:h-[115px] md:top-6 md:left-10 lg:w-[120px] lg:h-[138px] lg:top-2 lg:left-20 flex justify-center items-center shadow-xl z-20"
              >
                          ...
                          {item.dropdown.map((subItem, subIndex) => (
                            <li key={subItem.name} className="relative">
                              ...
                              {subItem.subDropdown && activeSubDropdown === subIndex && (
                                <ul className="absolute right-0 mt-1 w-40 bg-customBlue shadow-lg z-50">
                                  {subItem.subDropdown.map((subSubItem) => (
                                    <li key={subSubItem.name}>
                                      <button
                                        onClick={() => handleNavigation(subSubItem.slug)}
                                        className={`block px-4 py-1 w-full text-left text-sm hover:bg-blue-800 ${
                          ...
                          <ul className="bg-blue-800 w-full">
                            {subItem.subDropdown.map((subSubItem) => (
                              <li key={subSubItem.name}>
                                <button
                                  onClick={() => handleNavigation(subSubItem.slug)}
                                  className={`block px-8 py-1 w-full text-left text-xs ${
                                    location.pathname === subSubItem.slug ? 'font-bold' : ''
                                  }`}
                                >
                                  {subSubItem.name}

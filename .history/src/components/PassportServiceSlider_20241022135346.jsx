      ...
      {slides.map((slide, index) => (
        ...
        >
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center text-center px-4 md:px-8 z-20">
            <h2 className="text-white text-2xl md:text-4xl font-bold mb-4 font-gothic">{slide.title}</h2>
            <p className="text-white text-lg md:text-xl mb-6 font-lora">{slide.description}</p>
            <button
              onClick={() => navigate(slide.path)}
              className="bg-customYellow text-customBlue font-semibold px-6 py-3 rounded-full shadow-lg hover:bg-customBlue hover:text-white transition-all duration-300"
            >
              {slide.buttonText}
            </button>
          </div>

  ...
  return (
    <>
      ...
      <header className="bg-customBlue text-customYellow">
        <Container>
          <div className="flex justify-between items-center py-2"> {/* Added padding here for vertical adjustment */}
            
            {/* Logo and Company Name */}
            <div className="flex items-center">
              <button onClick={() => navigate('/')} className="focus:outline-none">
                <Logo className="w-20 h-20 object-contain" /> {/* Adjusted logo size */}
              </button>
              <div className="ml-4 text-customYellow">
                <p className="text-2xl lg:text-3xl font-semibold">D2 Global</p>
                <p className="text-lg lg:text-xl">Education and Immigration</p>
              </div>
            </div>

            {/* Desktop Menu */}
            <ul className="hidden lg:flex ml-auto space-x-8 items-center"> {/* Flex and space adjustments for vertical alignment */}
              {navItems.map((item, index) => (
                <li key={item.name} className="relative">
                  <button
                    ...
                    className={`underline-slide px-4 py-2 text-lg lg:text-xl ${location.pathname === item.slug ? 'active-page' : 'text-customYellow'}`}
                  >
                    {item.name}
                  </button>
                      ...
                      {item.dropdown.map((subItem) => (
                        <li key={subItem.name}>
                          <button
                            ...
                            className={`block px-4 py-2 hover:underline w-full text-left text-lg lg:text-xl ${location.pathname === subItem.slug ? 'font-bold' : ''}`}
                          >
                            {subItem.name}
                          </button>
                        </li>

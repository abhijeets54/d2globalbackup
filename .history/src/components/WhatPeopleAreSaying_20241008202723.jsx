...
const FloatingIcon = ({ Icon, left, top }) => (
  <motion.div
    className="absolute text-customYellow opacity-40 z-50"
...
const FloatingTestimonials = () => {
  ...
  return (
    <div className="py-12 bg-customBlue flex flex-col h-auto md:h-96 justify-center items-center overflow-hidden relative z-0">
      {/* Floating Icons */}
      ...
      >
        <h2 className="text-3xl md:text-4xl lg:text-5xl font-bold text-customYellow">
      ...
      <div className="relative w-full max-w-7xl mx-auto px-4">
        <div className="flex flex-col md:flex-row justify-center items-center h-auto md:h-[600px] lg:h-[700px]">
          {testimonials.map((testimonial, index) => (
            <motion.div
              key={testimonial.id}
              className={`absolute w-80 md:w-96 bg-white z-50 rounded-lg p-6 shadow-lg flex flex-col justify-center items-center text-center ${
                index % 2 === 0 ? 'md:left-0' : 'md:right-0' // Position based on even/odd index
              }`}
              initial={{ y: 0 }}
              animate={{
                y: index % 2 === 0
                  ? [-20, 20] // Even indexed items move up then down
                  : [20, -20], // Odd indexed items move down then up
              }}
              transition={{ duration: 2, repeat: Infinity, yoyo: true }} // Infinite animation with yoyo effect
            >
              <h3 className="text-lg md:text-xl font-semibold mb-2 text-customBlue">{testimonial.name}</h3>

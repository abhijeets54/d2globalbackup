  ...
  useEffect(() => {
    ...
    gsap.fromTo(
      ...
      { x: '-300px', scale: 0.8 }, // Start with a smaller plane size
      {
        ...
        ease: 'power3.inOut',
        onComplete: () => {
          // Animate the info text after the plane arrives
          gsap.to(infoText, {
            opacity: 1,
            ...
            stagger: 0.5,
            onComplete: () => {
              infoText.forEach((el) => el.classList.remove('opacity-0'));
            },
          });

          // Move the plane to the right and zoom out, without affecting the text
          gsap.to(plane, {

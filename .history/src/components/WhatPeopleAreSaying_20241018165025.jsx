...
import { GraduationCap, Book, Globe, Star } from 'lucide-react'; // Import Star icon
...
const TestimonialCircle = ({ testimonial, index, currentIndex }) => {
  ...
  return (
    <motion.div
      ...
      animate={{
        x: `${position * 300}px`, // Adjust spacing based on larger circle size
        scale: isCenter ? 1.5 : 0.8, // Slightly increase the center circle scale
    ...
    >
      ...
      <div className={`bg-white text-customBlue p-6 rounded-full w-64 h-64 border-3 border-customBlue flex flex-col justify-center items-center text-center shadow-xl ${isCenter ? 'z-10' : 'z-0'}`}>
        ...
        <div className="flex space-x-1 mb-2">
          {[...Array(5)].map((_, i) => (
            <Star key={i} size={16} className="text-yellow-400" /> // Golden stars
          ))}
        </div>
...
const FloatingTestimonials = () => {
  ...
  useEffect(() => {
    const timer = setInterval(() => {
      setCurrentIndex((prevIndex) => (prevIndex + 1) % testimonials.length); // Loop back to 0 after the last index
    }, 5000);

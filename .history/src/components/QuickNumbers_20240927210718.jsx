...
function QuickNumbers() {
  ...
  return (
    <div ref={quickNumbersRef} className="bg-customBlue py-8 w-full">
      <h2 className="text-center text-3xl sm:text-5xl font-extrabold text-customYellow pt-5 mb-8 tracking-widest">
        QUICK NUMBERS
      </h2>
        ...
        {data.map((item, index) => (
          <div key={index} className="text-center text-customYellow flex flex-col items-center">
            <div className="quicknumber flex flex-col justify-center items-center">
              ...
              <p className="mt-2 text-base sm:text-xl font-semibold font-serif">{item.label}</p>
            </div>
          </div>
        ))}
      </div>

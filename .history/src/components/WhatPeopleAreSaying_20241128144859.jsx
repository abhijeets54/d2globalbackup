...
const FloatingIcon = ({ Icon, position }) => (
  <motion.div
    className={`absolute text-customYellow ${position}`}
    animate={{ y: [0, -20, 0] }}
    transition={{ duration: 4, repeat: Infinity, repeatType: "reverse" }}
  >
    <Icon size={32} className="md:size-48" />
  </motion.div>
);
    ...
    >
      <div className={`bg-white text-customBlue p-4 md:p-8 rounded-full w-48 h-48 md:w-64 md:h-64 border-3 mt-3 border-customYellow flex flex-col justify-center items-center text-center shadow-xl ${isCenter ? 'z-10' : 'z-0'}`}>
        <div className="flex space-x-1">
          {[...Array(5)].map((_, i) => (
            <Star key={i} size={16} className="text-customYellow fill-yellow-400" />
          ))}
        </div>
        <h3 className="font-bold mb-2 text-xs md:text-sm">{testimonial.name}</h3>
        <div className="overflow-y-auto h-24 md:h-32 scrollbar-hide px-2">
...
const FloatingTestimonials = () => {
  ...
  return (
    <div className="relative bg-customBlue text-customYellow py-8 md:py-16 px-4 overflow-hidden h-[400px] md:h-[600px]">
      ...
      <h2 className="text-2xl md:text-4xl font-bold text-center mb-8 md:mb-12">WHAT OUR CLIENTS SAY ABOUT US</h2>

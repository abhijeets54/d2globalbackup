...
const people = [
  ...
  { name: 'Ms. Shanu', designation: 'IELTS/PTE Coordinator', image: '/shanu.webp' }
];

const People = () => {
  return (
    <div className="people-container bg-blue-950 text-yellow-400 px-4 sm:px-6 md:px-8 py-6">
      {/* Left text section */}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 sm:gap-6">
        {people.map((person, index) => (
          <div
            key={index}
            className="flex flex-col items-center bg-white rounded-md shadow-lg p-2 sm:p-3"
          >
            <div className="w-28 h-28 sm:w-32 sm:h-32">
              <img
                src={person.image}
                alt={person.name}
                className="rounded-md w-full h-full object-contain mb-2"
              />
            </div>

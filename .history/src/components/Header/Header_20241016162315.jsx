  ...
  return (
    <>
      ...
      <header className="bg-customBlue text-customYellow relative">
        <Container>
          <div className="flex justify-between items-center relative" style={{ height: '120px' }}> {/* Increased height */}
            
            {/* Logo and Company Name */}
            <div className="flex items-center relative flex-shrink-0 ml-8">
              {/* Logo container with absolute positioning */}
              <div className="relative" style={{ zIndex: 10 }}> {/* Ensure the logo appears above other elements */}
                <button
                  onClick={() => navigate('/')}
                  className="focus:outline-none"
                  style={{
                    position: 'absolute',
                    top: '50%', // Moves it down halfway
                    transform: 'translateY(-50%)', // Centers it vertically
                    zIndex: 10, // Ensures visibility above other elements
                  }}
                >
                  <img src="/path-to-your-logo/logo.png" alt="D2 Global Logo" className="w-16 h-16 object-contain" /> {/* Adjust the logo path and size */}
                </button>
              </div>

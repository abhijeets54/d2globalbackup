...
const FloatingIcon = ({ Icon, position }) => (
  <motion.div
    className={`absolute text-yellow-400 ${position}`}
    animate={{ y: [0, -20, 0] }}
...
const TestimonialCircle = ({ testimonial, index, currentIndex, totalItems }) => {
  ...
  return (
    ...
    >
      <div className={`bg-white text-customBlue p-8 rounded-full w-64 h-64 border-3 mt-3 border-customYellow flex flex-col justify-center items-center text-center shadow-xl ${isCenter ? 'z-10' : 'z-0'}`}>
...
const FloatingTestimonials = () => {
  ...
  return (
    <div className="relative bg-customBlue text-customYellow py-16 px-4 overflow-hidden h-[600px] lg:h-auto">
      <FloatingIcon Icon={GraduationCap} position="top-1/4 left-1/4" />
      <FloatingIcon Icon={Book} position="bottom-1/4 left-1/3" />
      <FloatingIcon Icon={Globe} position="bottom-1/4 right-1/3" />

      <h2 className="text-4xl font-bold text-center mb-12">WHAT OUR CLIENTS SAY ABOUT US</h2>

      <div className="relative w-full h-2/3 lg:h-auto flex justify-center items-center">

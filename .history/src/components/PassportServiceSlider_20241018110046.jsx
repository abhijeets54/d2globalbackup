...
const PassportServiceSlider = () => {
  ...
  const handleButtonClick = useCallback((action, title) => {
    console.log(`Button clicked for ${title}`);
    setFeedbackMessage(`Action triggered for ${title}`);
  ...
  const renderSlide = useCallback((index) => {
    const slides = [
      ...
      },
      {
        ...
        buttonAction: () => {
          console.log("Opening tourism website");
      ...
      >
        ...
        >
          <div className="w-full md:w-1/2 text-white space-y-4">
            ...
            <button
              onClick={() => handleButtonClick(slide.buttonAction, slide.title)}
  ...
  return (
    <div className="relative w-full h-[700px] overflow-hidden">
      ...
      {/* Feedback Message */}
      {feedbackMessage && (
        <div className="absolute top-4 left-1/2 transform -translate-x-1/2 bg-white text-black px-4 py-2 rounded-md shadow-md z-50">
          {feedbackMessage}
        </div>
      )}

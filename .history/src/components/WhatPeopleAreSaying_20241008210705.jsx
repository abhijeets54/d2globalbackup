      ...
      <div className="relative w-full max-w-7xl mx-auto px-4">
        <div className="flex justify-center items-center h-[550px] md:h-[600px] lg:h-[650px]">
          ...
          {testimonials.map((testimonial, index) => (
            <motion.div
              key={testimonial.id}
              className={`absolute w-80 h-80 md:w-96 md:h-96 bg-white rounded-full py-8 px-20 shadow-lg flex flex-col justify-center items-center text-center ${
                index === 0 ? 'left-4 md:left-8 lg:left-16' : 'right-4 md:right-8 lg:right-16'
              }`}
              animate={{
                y: index === 0 ? [-40, 40] : [40, -40],
              }}
              transition={{
                y: {
                  duration: 2,

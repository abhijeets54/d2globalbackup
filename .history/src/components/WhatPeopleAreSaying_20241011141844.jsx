  ...
  useEffect(() => {
    const timer = setInterval(() => {
      setCurrentIndices(prevIndices => {
        const newIndices = [...prevIndices.slice(1), (prevIndices[2] + 1) % testimonials.length];
        return newIndices;
      });
  ...
  return (
    <div className="relative bg-customBlue text-white py-16 px-4 overflow-hidden">
      ...
      <div className="flex justify-center items-center h-[450px]">
        <div className="flex space-x-8 relative" style={{ width: '1200px' }}>
          <AnimatePresence initial={false}>
            {currentIndices.map((index, i) => (
              <motion.div
                key={index}
                className="absolute top-0"
                initial={{ opacity: 0, left: '800px' }}
                animate={{ opacity: 1, left: `${i * 400}px` }}
                exit={{ opacity: 0, left: '-400px' }}
                transition={{ duration: 0.5, type: 'tween' }}
              >
                <motion.div
                  className="bg-white text-customBlue p-6 rounded-full w-96 h-96 flex flex-col justify-center items-center text-center shadow-lg"
                  animate={{ 
                    y: [0, -10, 0],
                  }}
                  transition={{
                ...
                >
                  <h3 className="font-bold mb-4 text-xl">{testimonials[index].name}</h3>
                  <div className="overflow-y-auto h-64 scrollbar-hide flex items-center">

          ...
          <div className="flex justify-between items-center">
            ...
            <div className="flex items-center flex-shrink-0">
              <button onClick={() => navigate('/')} className="focus:outline-none">
                <Logo className="w-20 h-20 object-contain" /> {/* Slightly larger logo */}
              </button>
              <div className="ml-3 text-customYellow text-left"> {/* Adjusted margin and text size */}
                <p className="text-2xl font-semibold">D2 Global</p> {/* Larger text */}
                <p className="text-lg">Education and Immigration</p> {/* Larger text */}
              </div>
            </div>

            {/* Contact Info and Social Media - Single Line */}
            <div className="flex items-center space-x-10"> {/* Reduced space between icons */}
              {/* Social Media Icons */}
              ...
              <div className="flex items-center space-x-6"> {/* Reduced space between email and phone */}
                ...
                <ul className="flex flex-col items-center space-y-4 py-6">
                  {navItems.map((item, index) => (
                    <li key={item.name}>
                      ...
                      {item.dropdown && activeDropdown === index && (
                        <>
                          <ul className="bg-customBlue shadow-lg z-50 text-customYellow">
                            {item.dropdown.map((subItem) => (
                              <li key={subItem.name}>
                                <button
                                  onClick={() => handleNavigation(subItem.slug)}

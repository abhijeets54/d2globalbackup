  ...
  return (
    <>
      ...
      <header className="bg-customBlue text-customYellow">
        <Container>
          <div className="flex justify-between items-center py-2"> {/* Reduced padding here */}
            ...
            <div className="flex items-center flex-shrink-0 ml-4"> {/* Reduced margin */}
              ...
              <div className="ml-2 text-customYellow text-left">
                ...
                <p className="text-sm lg:text-lg">Education and Immigration</p> {/* Reduced font size */}
              </div>
            </div>

            {/* Contact Info and Social Media */}
            <div className="flex items-center space-x-4 mr-60"> {/* Adjusted margin */}
              <div className="flex flex-col items-center">
                <p className="text-xs">Email Us</p> {/* Reduced text size */}
                <p className="flex items-center space-x-2 text-sm">
                  <FaEnvelope className="inline-block text-xl" />
                  <a href="mailto:info@d2global.com">info@d2global.com</a>
                </p>
              </div>
              <div className="flex flex-col items-center">
                <p className="text-xs">Call Us</p> {/* Reduced text size */}
                <p className="flex items-center space-x-2 text-sm">
                  <FaPhone className="inline-block text-xl rotate-90" />
                  <a href="tel:+919878990455">+91 98789 90455</a>
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="py-1 flex justify-between items-center"> {/* Reduced padding */}
            ...
            <div className="lg:hidden">
              <button onClick={toggleMenu} className="text-customYellow focus:outline-none">
                {menuOpen ? <FaTimes size={24} /> : <FaBars size={24} />} {/* Reduced icon size */}
              </button>
            </div>

            {/* Desktop Menu */}
            <ul className="hidden lg:flex ml-auto space-x-4"> {/* Adjusted spacing */}
              {navItems.map((item, index) => (
                <li key={item.name} className="relative">
                  <button
                    ...
                    className={`underline-slide inline-block px-4 py-1 text-sm lg:text-lg duration-200 ${location.pathname === item.slug ? 'active-page' : 'text-customYellow'}`}
                  >
                    {item.name}
                  </button>
                      ...
                      {item.dropdown.map((subItem) => (
                        <li key={subItem.name}>
                          <button
                            ...
                            className={`block px-4 py-2 hover:underline w-full text-left text-sm lg:text-lg ${location.pathname === subItem.slug ? 'font-bold' : ''}`}
                          >
                            {subItem.name}
                          </button>
                        </li>
            ...
            {menuOpen && (
              <div className="lg:hidden absolute top-16 left-0 w-full bg-customBlue shadow-md z-40 text-customYellow">
                          ...
                          <ul className="bg-customBlue shadow-lg z-50 w-full text-customYellow">
                            {item.dropdown.map((subItem) => (
                              <li key={subItem.name}>
                                <button
                                  onClick={() => handleNavigation(subItem.slug)}
                                  className={`block px-4 py-2 hover:underline w-full text-left text-lg ${location.pathname === subItem.slug ? 'font-bold' : ''}`}
                                >
                                  {subItem.name}
                                </button>
                              </li>
                ...
                <button onClick={toggleMenu} className="text-customYellow px-4 py-2 mt-4">
                  <FaTimes size={24} /> {/* Reduced icon size */}
                </button>
              </div>
            )}
          </div>

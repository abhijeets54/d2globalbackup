...
import { Star, GraduationCap, Book, Globe, Heart, Smile, ThumbsUp, Award } from 'lucide-react';
...
const FloatingTestimonials = () => {
  ...
  return (
    <div className="relative bg-customBlue text-white py-16 px-4 overflow-hidden">
      ...
      <FloatingIcon Icon={Heart} left={50} top={5} />
      <FloatingIcon Icon={Smile} left={70} top={90} />
      <FloatingIcon Icon={ThumbsUp} left={30} top={95} />
      <FloatingIcon Icon={Award} left={95} top={50} />
      ...
      <div className="flex justify-center">
        <AnimatePresence mode="wait">
          ...
          >
            {testimonials.slice(currentIndex * 3, currentIndex * 3 + 3).map((testimonial) => (
              <motion.div
                key={testimonial.id}
                className="bg-white text-customBlue p-6 rounded-full w-64 h-64 flex flex-col justify-center items-center text-center shadow-lg"
                animate={{ 
                  y: [0, -10, 0],
                }}
                transition={{
              ...
              >
                ...
                <p className="text-sm overflow-y-auto h-40 scrollbar-hide">{testimonial.text}</p>
              </motion.div>
            ))}
          </motion.div>
        </AnimatePresence>
